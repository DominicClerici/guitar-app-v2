...
> Adapted from [gabrielobholz/corner-smoothing-skill](https://github.com/gabrielobholz/corner-smoothing-skill). The React Native section below is Rumor-specific.
...
| Need                     | Use                                                                            |
| ------------------------ | ------------------------------------------------------------------------------ |
| React Native (this repo) | `react-native-svg` `<Path d={...}>` from the shared path generator — see below |
...
## React Native (rumor-mobile-expo)

This repo is React Native, so `clip-path`/Canvas do not apply. The path generator in [implementation.md](references/implementation.md) is pure JS and works unchanged — feed its output to `react-native-svg`:

- Render the shape with `<Svg>` + `<Path d={appleCornerPath({...})} />` (the repo already depends on `react-native-svg`), sized via `onLayout` or known dimensions.
- Per AGENTS.md, do not scatter ad-hoc SVG in feature code: put the path generator in `src/lib/` and build one reusable primitive under `src/components/ui/` (e.g. a `SquircleView` that renders the SVG as an absolute-fill background behind its children), then compose it everywhere.
- Content that must be clipped to the shape (images, gradients) can use the same path inside an SVG `<ClipPath>`/`<Mask>`.
- When animating size, regenerate the path from animated dimensions (Reanimated `useAnimatedProps` on the `Path`); do not animate `borderRadius` and call it smoothing.

 ...
 *
 * A CSS/RN `border-radius` corner is a quarter circle bolted onto two straight
 * edges: curvature jumps from zero to 1/r at the join, and the eye reads that
 * discontinuity as a seam. Apple's corner (a "squircle", `CALayer.cornerCurve =
 * .continuous` since iOS 13) instead eases curvature in and out, so the corner
 * starts further back along the edge and the join disappears.
 *
 * The true shape is not a superellipse — it is a piecewise cubic Bézier fit,
 * reverse-engineered by Figma and published as "Desperately seeking squircles".
 * Each corner is three pieces: a Bézier ramping curvature up, a circular arc of
 * reduced sweep through the apex, and a mirrored Bézier ramping it back down.
 *
...
export function squirclePath({
  ...
  return [
    `M ${n(x + width - p)} ${n(y)}`,
    // top-right: ramp up, arc through the apex, ramp down
